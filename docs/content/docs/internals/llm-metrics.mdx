...
| --- | --- | --- | --- |
| `ttft_ms` | ms | `u64` | Local LLM: `Instant` of the first emitted `Response::Chunk` minus the `generate()` start time. Cloud LLM: gateway round-trip time (`client.complete()` start → return). |
| `tokens_in` | count | `usize` | Local LLM (llama.cpp / MLX): prompt token count captured before the streaming loop so it survives mid-stream aborts. Local LLM (mistral): `Usage.prompt_tokens` from the terminal chunk. Cloud LLM: upstream `Usage.prompt_tokens` when populated. |
| `tokens_generated` | count | `usize` | Local LLM: emitted token count from the streaming telemetry recorder. Cloud LLM: surfaced as `tokens_out` instead — see below. |
| `tokens_out` | count | `usize` | Cloud LLM only, plus a local-span alias equal to `tokens_generated` for telemetry hoisting. The SDK's `extract_llm_token_counts` accepts both `tokens_out` and `tokens_generated` as aliases when hoisting to the platform-event payload top-level. |
| `tokens_per_second` | tok/s | `f32` | Wallclock: `tokens_generated / elapsed`. Includes prefill time. |
| `decode_tps` | tok/s | `f32` | mistralrs `Usage.avg_compl_tok_per_sec` or MLX decode-token throughput after the first emitted token. `None` when timing is below resolution. |
| `prefill_tps` | tok/s | `f32` | mistralrs `Usage.avg_prompt_tok_per_sec` or MLX prompt-token throughput through TTFT. Same zero-filter semantics. |
...
1. The selected local backend produces `GenerationOutput`. For mistralrs,
   `MistralBackend::generate` streams tokens via `stream_chat_request`; on
   `is_streaming=true`, mistralrs signals completion via the **last
   `Response::Chunk`**. For MLX SafeTensors, `runtime_adapter/mlx/generate.rs`
   emits `PartialToken`s from the MLX decode loop and derives TTFT,
   prefill/decode TPS, ITL, emitted-chunk count, and finish reason from that
   loop. A stream that closes without a terminal signal returns
   `AdapterError::InferenceFailed`; partial text is never published as a
   successful result.
2. `LlmRuntimeAdapter::execute`
   (`crates/xybrid-core/src/runtime_adapter/llm.rs`) writes each scalar into
...
deterministic decoding for local LLM backends that support it. Callers
that want sampling must explicitly set `temperature > 0.0`. This preserves
reproducible output for tests, caching, and any stage that depends on
stable local inference.

## Guarantees and non-guarantees

- **Guaranteed on every successful LLM generation**: `ttft_ms`,
  `tokens_in`, `tokens_generated` (local) / `tokens_out` (cloud),
  `tokens_per_second`, `emitted_chunks`, `finish_reason`.
- **Guaranteed even when generation aborts mid-stream** (e.g.
  `MemoryPressure::Critical` triggers cloud fallback): `tokens_in` is
  attached to the `llm_inference_streaming` span before the streaming
  loop starts, so the dashboard's `LocalAborted` row still carries the
  prompt token count. Other fields require successful completion.
- **Best-effort**: `decode_tps`, `prefill_tps`, `mean_itl_ms`,
  `p95_itl_ms`. Short prompts on fast hardware occasionally produce
  timing below the measurement resolution; these are filtered to `None`.
...
- `crates/xybrid-core/src/runtime_adapter/llm.rs` — backend abstraction,
  envelope metadata, tracing emission, and `GenerationConfig`.
- `crates/xybrid-core/src/runtime_adapter/mlx/generate.rs` — MLX
  SafeTensors generation loop and streaming metric derivation.
