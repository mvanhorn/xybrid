...
| `--backend` | | 强制指定本地生成或 Embedding 后端（`auto`、`mlx`、`llamacpp`、`mistral`） |
...
对于来自 Registry 的本地 LLM 阶段，`--backend mlx` 会请求
SafeTensors 变体，并要求 Apple Silicon 构建启用了可用的
`llm-mlx-runtime` 功能。`--backend llamacpp` 会为 LLM 阶段请求 GGUF
变体。Registry Embedding 阶段目前会拒绝显式的非 MLX 本地后端，因为本地
selector 还没有暴露非 MLX 的 Embedding 运行时。
...
### 强制使用 MLX SafeTensors

```bash
xybrid run --model qwen3-4b --backend mlx --input-text "Summarize this."
```

当 Registry 模型同时发布 GGUF 和 MLX SafeTensors 变体，并且你希望走
MLX 运行时而不是自动本地后端选择时，使用该选项。
