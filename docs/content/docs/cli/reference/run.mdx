...
| `--backend` | | Force a local generation or embedding backend (`auto`, `mlx`, `llamacpp`, `mistral`) |
...
For registry-backed local LLM stages, `--backend mlx` requests the
SafeTensors artifact variant and requires an Apple Silicon build with the
`llm-mlx-runtime` feature available. `--backend llamacpp` requests the GGUF
variant for LLM stages. Registry embedding stages currently reject explicit
non-MLX local backends because no non-MLX embedding runtime is exposed through
the local selector.
...
### Force MLX SafeTensors

```bash
xybrid run --model qwen3-4b --backend mlx --input-text "Summarize this."
```

Use this when a registry model publishes both GGUF and MLX SafeTensors
variants and you want the MLX runtime path instead of automatic local backend
selection.
