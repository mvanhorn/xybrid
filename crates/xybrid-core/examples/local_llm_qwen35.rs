//! Local LLM Inference - Qwen 3.5 0.8B (GGUF)
//!
//! This example demonstrates local LLM inference using the metadata-driven
//! execution system with Qwen 3.5 0.8B, Alibaba Cloud's lightweight model
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("═══════════════════════════════════════════════════════");
    println!("  Local LLM - Qwen 3.5 0.8B (GGUF)");
    println!("═══════════════════════════════════════════════════════");
    println!();

    // Resolve model directory using fixtures
    let model_dir = model_fixtures::require_model("qwen3.5-0.8b");
    ...
    let prompt = "Explain what makes Rust's ownership system unique in 2-3 sentences.";
    ...
    match &output_envelope.kind {
        ...
        EnvelopeKind::Embedding(_) => {
            println!("Unexpected: Got embedding output instead of text");
        }
        EnvelopeKind::Audio(_) => {
            println!("Unexpected: Got audio output instead of text");
        }
        EnvelopeKind::TokenIds(_) => {
            println!("Unexpected: Got raw token IDs output instead of text");
        }
    }
